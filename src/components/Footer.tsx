  ...
  return (
    <footer className="relative py-10 sm:py-12 lg:py-16 bg-background border-t border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className={`grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 mb-8 sm:mb-12 ${isRTL ? 'text-right' : ''}`}>
          {/* Logo & Description */}
          <div className="col-span-2 space-y-3 sm:space-y-4">
            <div className={`flex items-center gap-2 sm:gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              <div className={isRTL ? 'text-right' : 'text-left'}>
                <span className="text-white font-bold text-sm sm:text-lg block">Al Samer International</span>
                <span className="text-white/50 text-[10px] sm:text-xs">Company for Trade and Transport</span>
              </div>
            </div>
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            <ul className="space-y-1.5 sm:space-y-2">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <a href={link.href} className="text-xs sm:text-sm text-white/50 hover:text-white transition-colors">
                    {link.label}
                  </a>
                </li>
              ))}
        ...
        {/* Bottom Bar */}
        <div className={`pt-6 sm:pt-8 border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-4 ${isRTL ? 'sm:flex-row-reverse' : ''}`}>
          ...
          <div className={`flex items-center gap-4 sm:gap-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
            <a href="#" className="text-[10px] sm:text-xs text-white/40 hover:text-white/70 transition-colors">
              {t("footer.privacy")}
            </a>
            <a href="#" className="text-[10px] sm:text-xs text-white/40 hover:text-white/70 transition-colors">
              {t("footer.terms")}
